  ...
  return (
    <>
      <div className="countryHeaderDiv">
        ...
        <p className="country-info hidden md:block">Currencies Name</p>
              ...
              <div className="hidden md:block">
                {curncKeys.map((item, idx) => (
                  <div key={idx}>
                    ...
                    <p key={idx + 121} className="text-main">
                      {item.symbol}
                    </p>
                  </div>
                ))}
              </div>
              <div className="hidden lg:block">{langElements}</div>
              <p className="hidden lg:block">{country.population}</p>
              <p className="hidden lg:block">{country.area}</p>
              <div className="hidden md:block">
                <p>{country.region}</p>
                <p className="text-main bloc">{country.subregion}</p>
              </div>
